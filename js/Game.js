...
update: function () {
	...
	if (this.game.time.now > this.accelTimer){
		if (this.startTime != 0)
			...
			this.hourse2.x = this.hourse2.x + 10 + Math.random();
		if (this.speed >0 ){
		...
		if (this.rightKey.isDown || this.game.input.pointer1.isDown ){
			...
			if (this.speed <= this.gearArray[this.gear]   ){
				this.acceleration  = 	   this.gearAccel[this.gear]; //this.speed
